...
import { putObject } from './ossUtil.js';
...
// Tools that should automatically take screenshots after execution
...
export class Context {
  ...
  private async _takeAutoScreenshot(toolName: string): Promise<{ localPath: string, ossPath: string }[]> {
    if (!this.config.autoScreenShot || !AUTO_SCREENSHOT_TOOLS.includes(toolName) || !this._currentTab)
      return [];
    ...
    try {
      ...
      console.error('Failed to take auto screenshot:', error);
      return [];
    }
  }
  ...
  async run(tool: Tool, params: Record<string, unknown> | undefined) {
    ...
    let screenshotResult: { localPath: string, ossPath: string }[] = [];
    ...
    try {
      ...
      // Take automatic screenshot for whitelisted tools
      screenshotResult = await this._takeAutoScreenshot(tool.schema.name);
    } finally {
